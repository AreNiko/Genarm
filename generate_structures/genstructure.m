function [vG, vGc, vGextC, vGextF, vGstayOff] = genstructure(env, origo, max_radius, wrist_radius, height, thickness, arm_height)
    ...
    vGextF(1:origo(1), origo(2)-1:origo(2)+1, 1:b_layer) = vG(1:origo(1), origo(2)-1:origo(2)+1, 1:b_layer);
    vGextF(origo(1)-1:origo(1)+1, 1:origo(2), 1:b_layer) = vG(origo(1)-1:origo(1)+1, 1:origo(2), 1:b_layer);
    vGextF(origo(1):end, origo(2)-1:origo(2)+1, 1:b_layer) = vG(origo(1):end, origo(2)-1:origo(2)+1, 1:b_layer);
    vGextF(origo(1)-1:origo(1)+1, origo(2):end, 1:b_layer) = vG(origo(1)-1:origo(1)+1, origo(2):end, 1:b_layer);
    vGextF = vGextF - vGc;
    vGextF(vGextF<0) = 0;

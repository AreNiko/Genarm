...
def gen_data():
	...
	path = os.path.abspath(os.getcwd()) + "/data/reinforce_all/010_step" # _sf = start&finish, _step = start-finish
	...
	for i in range(20):
		print(i)
		radii = np.random.randint(12, 14)
		wrist_rad = radii - np.random.randint(1, 3)
		struct1og, vGc, vGextC, vGextF, vGstayOff = eng.get_struct1(radii,radii,wrist_rad,np.random.randint(50,100), nargout=5)
		...
		for j in range(50):
			true_struct = np.array(eng.reinforce_struct(matlab.int8(struct1.tolist()), vGextC,
														vGextF, vGstayOff, 100), dtype=np.int8)
			"""

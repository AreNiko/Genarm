...
def flip_coord(pi_old, struct):
	...
	x = np.floor(pi_old[:,0]*tf.cast(xdim,tf.float32),dtype=np.uint8)
	y = np.floor(pi_old[:,1]*tf.cast(ydim,tf.float32),dtype=np.uint8)
	z = np.floor(pi_old[:,2]*tf.cast(zdim,tf.float32),dtype=np.uint8)
	#x = tf.cast(x,tf.int32)
	#y = tf.cast(y,tf.int32)
	#z = tf.cast(z,tf.int32)
	print(np.concatenate((x.T, y.T, z.T), axis=1))

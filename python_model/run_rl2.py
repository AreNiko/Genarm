...
def flip_coord(pi_old, struct):
	...
	x = np.floor(pi_old[:,0]*tf.cast(xdim,tf.float32),dtype=int8)
	y = np.floor(pi_old[:,1]*tf.cast(ydim,tf.float32),dtype=int8)
	z = np.floor(pi_old[:,2]*tf.cast(zdim,tf.float32),dtype=int8)

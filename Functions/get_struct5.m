function [vG, vGextC, vGextF, vGstayOff] = get_struct5()
    ...
    voxSize = 1; % 1 mm på printern, kan forandres for bedre eller dårligere vokseloppløsning
    ...
    %%%%%%%%%%% Lager initiell vG
    vG = zeros(xDist1*2+xDist3, radiusAxel*20, 10, 'int8');
    yCenter = round(size(vG,2)/2);
    vG = vGbox(vG,[3 round(yCenter-radiusAxel*5/2) 2],1, size(vG,1)-4,radiusAxel*5,10);
    %%%%%%%%%%% Definerer låste voksler - blå
    vGextC = zeros(size(vG),'int8');
    vGextC = vGcylinder(vGextC,[xDist2,yCenter,2],1, 2,10);
    vGextC = vGcylinder(vGextC,[xDist3,yCenter,2],1, 2,10);
    %%%%%%%%%%% Definerer kraft voksler - grå
    vGextF = zeros(size(vG),'int8');
    vGextF = vGcylinder(vGextF,[xDist1,yCenter,2],1, radiusAxel,10);
    %%%%%%%%%%% Definerer stayOff voksler - gule
    vGstayOff = zeros(size(vG),'int8');
    vGstayOff = vGcylinder(vGstayOff,[xDist1,yCenter,2],1, radiusAxel+3,10);
    vGstayOff = vGcylinder(vGstayOff,[xDist2,yCenter,2],1, radiusAxel+3,10);
    vGstayOff = vGcylinder(vGstayOff,[xDist3,yCenter,2],1, radiusAxel+3,10);
    %{
